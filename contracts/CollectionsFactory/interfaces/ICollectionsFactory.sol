interface ICollectionsFactory {
    ...
    event CollectionCreated(uint256 id, address collection, address author, string json, bool isProtected);
    ...
    event NftCreated(uint256 id, address nft, address collection, address owner, address manager, address creator);
    ...
    function deployCollection(
        ...
        address sendGasTo,
        bool isProtected
    ) external returns (address addrCollection);
    ...
    function deploymentIsActive() external view responsible returns (bool isActive);
    ...
    function collectionCodeHash() external view responsible returns (uint256 codeHash);

    /// @notice Returns collection address by its ID
    function collectionAddress(uint256 id) external view responsible returns (address collection);
}
