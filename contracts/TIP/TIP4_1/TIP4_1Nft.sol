...
import "@itgold/everscale-tip/contracts/TIP4_1/interfaces/INftChangeOwner.sol";
...
contract TIP4_1Nft is ITIP4_1NFT {
    /**
     * Errors
     **/
    ...
    function transfer(
        ...
        for ((address dest, CallbackParams p): callbacks) {
            INftTransfer(dest).onNftTransfer{ value: p.value, flag: 0 + 1, bounce: false }(
                _id,
                oldOwner,
                to,
                _manager,
    ...
    function changeOwner(
        ...
        for ((address dest, CallbackParams p): callbacks) {
            INftChangeOwner(dest).onNftChangeOwner{ value: p.value, flag: 0 + 1, bounce: false }(
                _id,
                _manager,
                oldOwner,
                newOwner,
    ...
    function changeManager(
        ...
        for ((address dest, CallbackParams p): callbacks) {
            INftChangeManager(dest).onNftChangeManager{ value: p.value, flag: 0 + 1, bounce: true }(
                _id,
                _owner,
                oldManager,
                newManager,
    ...
    function getInfo()
        ...
        responsible
        override
        returns (uint256 id, address owner, address manager, address collection)
    {
