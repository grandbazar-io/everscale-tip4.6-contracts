...
// :: * -> Bool
const isValidPublicKey = (x) =>
  typeof x === "string" &&
  pubkeyRegex.test(strip0x(x)) &&
  !onlyZero.test(strip0x(x));
const isValidAddress = (x) =>
  typeof x === "string" &&
  addressRegex.test(x) &&
  !onlyZero.test(stripWorkchain(x));
...
const getAddressFileNamePart = () => {
  let fileNamePart;
  if (locklift.context.network.name == "mainnet") {
    fileNamePart = "main";
  } else if (locklift.context.network.name == "testnet") {
    fileNamePart = "dev";
  } else {
    fileNamePart = locklift.context.network.name;
  }
